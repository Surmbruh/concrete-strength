...
def _extract_weight_stats(weight) -> Tuple[float, float]:
    """Return (mu, sigma) for both Bayesian and classic weights."""
    if hasattr(weight, 'mu') and hasattr(weight, 'sigma'):
        return _to_float(weight.mu), _to_float(weight.sigma)
    if hasattr(weight, 'value'):
        return _to_float(weight.value), 0.0
...
def draw_genome(genome: 'Genome',
    ...
    if enabled_conns:
        stats = [_extract_weight_stats(c.weight) for c in enabled_conns]
        mus = [m for m, _ in stats]
        sigmas = [s for _, s in stats]
    ...
    for connection in genome.connections:
        ...
        mu, sigma = _extract_weight_stats(connection.weight)
...
def draw_weight_distributions(genome: 'Genome',
    ...
    stats = [_extract_weight_stats(c.weight) for c in enabled]
    mus = [m for m, _ in stats]
    sigmas = [s for _, s in stats]
