...
# Чекпоинты и эксперименты — всё в репо
CHECKPOINT_DIR = os.path.join(os.getcwd(), "checkpoints")
EXPERIMENTS_DIR = os.path.join(os.getcwd(), "experiments")
os.makedirs(EXPERIMENTS_DIR, exist_ok=True)

n_pt = len([f for f in os.listdir(CHECKPOINT_DIR) if f.endswith('.pt')]) if os.path.exists(CHECKPOINT_DIR) else 0
n_json = len([f for f in os.listdir(CHECKPOINT_DIR) if f.endswith('.json')]) if os.path.exists(CHECKPOINT_DIR) else 0
print(f"Working dir: {os.getcwd()}")
print(f"Checkpoints: {CHECKPOINT_DIR} ({n_pt} моделей, {n_json} результатов)")
